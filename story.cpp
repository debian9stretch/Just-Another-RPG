...
int main(int argc, char const *argv[]) {
	...
	while (window.isOpen()) {
		...
		while (window.pollEvent(event)) {
			...
			//create SFML type Font and Text objects so a font can be loaded, then used to stream text in an SFML window
			sf::Font font;

			sf::Text text;

			font.loadFromFile("Azonix.tff");

			try {
				if (!font.loadFromFile("Azonix.tff")) {
					throw - 1;
				}
			}
			catch (int e) {
				std::cout << "An error has occured and the file could not be found. Error code: " << e;
			}
